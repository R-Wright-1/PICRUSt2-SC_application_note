...
Reformat raxml files for sepp:
```{bash, eval=FALSE}
cp archaea_0.8_raxml.raxml.log archaea_0.8_raxml.raxml_info.log
python sepp_reformat-info.py archaea_0.8_raxml.raxml_info.log
```
Using this script: https://github.com/smirarab/sepp/blob/master/sepp-package/buildref/reformat-info.py saved as: sepp_reformat-info.py

Didn't work, so going to work through it: Note that the values here seem to change slightly with every run. 
Some notes on making this manually:
Base frequencies - these are already in the order required (ACGT)
Overall time for tree evaluation can be replaced with Elapsed time
...
Partition: 0
Alignment Patterns: 1340
Name: No Name Provided
DataType: DNA
Substitution Matrix: GTR

RAxML-NG was called at 04-Dec-2024 09:15:27 as follows:

raxml-ng --evaluate --msa archaea_0.8_raxml-check.raxml.reduced.phy --tree archaea_16S_centroids_ssu_align_best_reduced_0.8_fixed.tre --p
refix archaea_0.8_raxml --model GTR+G <E2><80><94>threads 24

Base frequencies: 0.224411 0.252166 0.284495 0.238928

Inference[0]: Time 69.721592 CAT-based likelihood -0000, best rearrangement setting 5
alpha[0]: 1.000000 rates[0] ac ag at cg ct gt: 1.166108 4.368360 1.832145 1.019301 5.962743 1.000000


NOT conducting any final model optimizations on all 1 trees under CAT-based
model ....

Final GAMMA  likelihood: -216263.676941
```
Saved as arc_ref.raxml_info
...
Partition: 0
Alignment Patterns: 1577
Name: No Name Provided
DataType: DNA
Substitution Matrix: GTR

RAxML-NG was called at 04-Dec-2024 09:16:05 as follows:

raxml-ng --evaluate --msa bacteria_0.8_raxml-check.raxml.reduced.phy --tree bacteria_16S_centroids_ssu_align_best_reduced_0.8_fixed.tre -
-prefix bacteria_0.8_raxml --model GTR+G --threads 24

Base frequencies: 0.209289 0.232667 0.311531 0.246513 

Inference[0]: Time 896.452 CAT-based likelihood -0000, best rearrangement setting 5
alpha[0]: 1.000000 rates[0] ac ag at cg ct gt: 1.048719 2.928148 1.638867 0.831209 3.689522 1.000000 


NOT conducting any final model optimizations on all 1 trees under CAT-based
model ....

Final GAMMA  likelihood: -4679887.038556
```
Saved as bac_ref.raxml_info
...
### Did I even use the right versions of the files?!?! I don't think so!!! But maybe not necessary??

raxmlHPC-PTHREADS-SSE3 -g archaea_16S_centroids_ssu_align_best_reduced_0.8_fixed.tre -s archaea_0.8_raxml-check.raxml.reduced.phy -m GTRGAMMA -n archaea_16S_centroids_ssu_align_best_reduced_0.8_fixed_SSE3_PTHREADS -p 13
```
...
cp gtdb_picrust_files/arc_ref.raxml_info gtdb_picrust_ref/arc_ref
```
...
place_seqs.py -s chemerin_16S/seqs_arc.fna -o arc_placed_seqs.tre -p 24 -r arc \
                    --intermediate placement_working_sepp_arc -t sepp --verbose
                    
place_seqs.py -s chemerin_16S/seqs_arc.fna -o arc_placed_seqs_epa.tre -p 24 -r arc \
                    --intermediate placement_working_sepp_arc --verbose
                    
#the new file kind of works... had to run export LC_ALL=C
#vi /bigpool/robyn/picrust2_database/picrust2-v2.6.0/picrust2/default_files/archaea/arc_ref/arc_ref.raxml_info

hsp.py -i 16S -r archaea -t arc_placed_seqs.tre -o arc_marker_nsti_predicted.tsv.gz -p 1 -n
hsp.py -i 16S -r archaea -t arc_placed_seqs_raxml_info_separate_run.tre -o arc_raxml_info_separate_run_marker_nsti_predicted.tsv.gz -p 1 -n
hsp.py -i 16S -r archaea -t arc_placed_seqs_epa.tre -o arc_epa_marker_nsti_predicted.tsv.gz -p 1 -n

vi /bigpool/robyn/picrust2_database/picrust2-v2.6.0/picrust2/default_files/bacteria/bac_ref/bac_ref.raxml_info

place_seqs.py -s chemerin_16S/seqs_arc.fna -o bac_placed_seqs.tre -p 24 \
                    --intermediate placement_working_sepp_bac -t sepp --verbose
                    
place_seqs.py -s chemerin_16S/seqs_arc.fna -o bac_placed_seqs_epa.tre -p 24 \
                    --intermediate placement_working_sepp_bac --verbose
                    
hsp.py -i 16S -r bacteria -t bac_placed_seqs.tre -o bac_marker_nsti_predicted.tsv.gz -p 1 -n
hsp.py -i 16S -r bacteria -t bac_placed_seqs_epa.tre -o bac_epa_marker_nsti_predicted.tsv.gz -p 1 -n
```
...
Place seqs:
```{bash, eval=FALSE}
place_seqs.py -s test_data/place_seqs/study_seqs_test.fasta -r bac -o test_data/place_seqs/place_seqs_output_MPGA/bac_placed_seqs.tre --intermediate test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_bac

place_seqs.py -s test_data/place_seqs/study_seqs_test.fasta -r arc -o test_data/place_seqs/place_seqs_output_MPGA/arc_placed_seqs.tre --intermediate test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_arc

gzip test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_bac/*.fasta 
gzip test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_arc/*.fasta 
#gunzip test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_bac/*.fasta.gz
#gunzip test_data/place_seqs/place_seqs_output_MPGA/place_seqs_working_arc/*.fasta.gz

place_seqs.py -s test_data/place_seqs/seqs_to_align.fasta -r bac -o test_data/place_seqs/place_seqs_output_seqs_to_align/bac_placed_seqs.tre --intermediate test_data/place_seqs/place_seqs_output_seqs_to_align/place_seqs_working_bac

place_seqs.py -s test_data/place_seqs/seqs_to_align.fasta -r arc -o test_data/place_seqs/place_seqs_output_seqs_to_align/arc_placed_seqs.tre --intermediate test_data/place_seqs/place_seqs_output_seqs_to_align/place_seqs_working_arc

cp test_data/place_seqs/place_seqs_output_seqs_to_align/place_seqs_working_bac/study_seqs_hmmalign.fasta test_data/place_seqs/hmmalign_out_bac.fasta
cp test_data/place_seqs/place_seqs_output_seqs_to_align/place_seqs_working_arc/study_seqs_hmmalign.fasta test_data/place_seqs/hmmalign_out_arc.fasta

rm -r test_data/place_seqs/place_seqs_output_seqs_to_align
```

Make md5sum default files:
```{bash, eval=FALSE}
cd ..
cd picrust2/default_files/bacteria 
md5 bac_ref/bac_ref.fna 
MD5 (bac_ref/bac_ref.fna) = 0903e782b6248d0528f6339cbe139477
md5 bac_ref/bac_ref.tre
MD5 (bac_ref/bac_ref.tre) = bb5ebff1d6c3631b3c5c231111c4c927
md5 bac_ref/bac_ref.hmm
MD5 (bac_ref/bac_ref.hmm) = 4d733eafcdfb82015025c1307729be4b
md5 bac_ref/bac_ref.model
MD5 (bac_ref/bac_ref.model) = a526202df3b072403da6e22efed3ea5a

cd ..
cd archaea
md5 arc_ref/arc_ref.fna
MD5 (arc_ref/arc_ref.fna) = de8a9fd11a2753c068842a5ae576e981
md5 arc_ref/arc_ref.tre
MD5 (arc_ref/arc_ref.tre) = 6081bbebf1e87fca1226c781ba3f07ac
md5 arc_ref/arc_ref.hmm
MD5 (arc_ref/arc_ref.hmm) = 0d0174451eb8a12df1b972ea7c88d670
md5 arc_ref/arc_ref.model
MD5 (arc_ref/arc_ref.model) = 3483e5d72ca2cd5bc9a74657beff780c
```

Aborted this because they were annoying and mainly running only to make sure the basic functions themselves are working, not to check the database as such. Many are using subsampled files anyway, so the basic working is the same. 

Final install test vulcan:
```{bash, eval=FALSE}
#conda remove -n picrust2-v2.6.0-finaltest --all
#rm -r picrust2-v2.6.0

cd tools
git clone --single-branch -b PICRUSt2-v2.6.0 https://github.com/picrust/picrust2
mv picrust2 picrust2-v2.6.0
cd picrust2-v2.6.0
conda env create -n picrust2-v2.6.0-finaltest -f picrust2-env.yaml
conda activate picrust2-v2.6.0-finaltest
pip install --editable .

cd ..
mkdir picrust2_testing
cp -r /bigpool/robyn/picrust2_database/test_sepp/chemerin_16S/ picrust2_testing/
cd picrust2_testing
mkdir test_output
...
/usr/bin/time -v \
    picrust2_pipeline.py -s chemerin_16S/seqs_arc.fna \
                         -i chemerin_16S/table.biom \
                         -o test_output/v2.6.0_out_epang \
                         -p $THREADS \
                         --stratified \
                         --per_sequence_contrib \
                         -t epa-ng \
    2> test_output/v2.6.0_out_epang.log
    
/usr/bin/time -v \
    picrust2_pipeline.py -s chemerin_16S/seqs_arc.fna \
                         -i chemerin_16S/table.biom \
                         -o test_output/v2.6.0_out_sepp \
                         -p $THREADS \
                         --stratified \
                         --per_sequence_contrib \
                         -t sepp \
    2> test_output/v2.6.0_out_sepp.log
    
place_seqs.py -s chemerin_16S/seqs_arc.fna --ref_dir bac -o bac_placed_seqs.tre -p 1 --intermediate bac_placement_working -t sepp
place_seqs.py -s chemerin_16S/seqs_arc.fna --ref_dir arc -o arc_placed_seqs.tre -p 1 --intermediate arc_placement_working -t sepp
```

Final install test local:
```{bash, eval=FALSE}
cd testing_installs
...
#conda remove -n picrust2-2.6.0-github --all
conda env create -n picrust2-2.6.0-github -f picrust2-env.yaml --platform osx-64 

#remove sepp, r-castor, hmmer, epa-ng, dendropy lines
...
cd tools/picrust2-v2.6.0
conda env update --name qiime2-amplicon-2024.10 --file picrust2-env.yaml
```

## Testing changing the dependencies
...
cd testing_installs/picrust2-2.6.0
#made some changes to the dependencies
#Notes: 

conda env create -f picrust2-env.yaml -n picrust2_dev --platform osx-64
conda activate picrust2_dev
pip install --use-pep517 --config-settings editable_mode=compat -e .
pytest
#all ran OK! Added some versions back in from the conda list
#now try again
conda env create -f picrust2-env.yaml -n picrust2_dev_2 --platform osx-64
conda activate picrust2_dev_2
pip install --use-pep517 --config-settings editable_mode=compat -e .
pytest
#passed!
```

Try again with bioconda environment: 
Following instructions here: https://github.com/bioconda/bioconda-common
```{bash, eval=FALSE}
...
conda activate /opt/mambaforge/envs/bioconda
conda clean --all
...
Simulate samples from the total list of sequences in our samples that have 10, 100, 1000, 10000 sequences and 10, 100, 1000, 10000 samples. Run this with old and new databases and get time and memory use.
Run all with 12 threads, but try one of them with only 1 thread. 

Just try running with /usr/bin/time:
```{bash, eval=FALSE}
(/usr/bin/time -v picrust2_pipeline.py -s chemerin_16S/seqs.fna -i chemerin_16S/table.biom -o picrust2_out_pipeline_time_test -p 1) 2> test_time.txt
```

Make new folder:
```{bash, eval=FALSE}
mkdir simulated_samples
```
...
all_seqs = {}
count = 0
for fa in ['blueberry_16S_rep_seqs.fasta', 'cameroon_16S_rep_seqs.fasta', 'hmp_16S_rep_seqs.fasta', 'indian_16S_rep_seqs.fasta', 'mammal_16S_rep_seqs.fasta', 'ocean_16S_rep_seqs.fasta', 'primate_16S_rep_seqs.fasta']:
  for record in SeqIO.parse('picrust_datasets_test/data/'+fa, 'fasta'):
    all_seqs[count] = record
    count += 1

num_seqs = [10, 100, 1000, 10000]
max_abun = [10000, 1000, 10, 10]
num_samples = [10, 100, 1000]

for n in range(len(num_seqs)):
  ns = num_seqs[n]
  #if ns > 10: continue
  list_nums = [b for b in all_seqs]
  random_seqs = random.sample(list_nums, ns)
  these_seqs = []
  first_row = ['#OTU_ID']
  for rs in random_seqs:
    these_seqs.append(all_seqs[rs])
    first_row.append(all_seqs[rs].id)
  SeqIO.write(these_seqs, 'simulated_samples/'+str(ns)+'_sequences.fasta', "fasta")
  for num_sam in num_samples:
    print(ns, num_sam)
    #if num_sam > 10: continue
    this_dataset = []
    for a in range(num_sam):
      this_sample = ['Sample_'+str(a+1)]
      for s in range(len(these_seqs)):
        this_sample.append(random.randrange(1, max_abun[n]))
      this_dataset.append(this_sample)
    this_dataset = pd.DataFrame(this_dataset, columns=first_row).set_index('#OTU_ID')
    this_dataset = this_dataset.transpose()
    this_dataset.to_csv('simulated_samples/'+str(ns)+'_sequences_'+str(num_sam)+'_samples.tsv', sep='\t')
...
```{bash, eval=FALSE}
cd simulated_samples
parallel -j 2 --progress '(/usr/bin/time -v picrust2_pipeline_oldIMG.py -s {1}_sequences.fasta -i {1}_sequences_{2}_samples.tsv -o picrust2_out_pipeline_{1}_sequences_{2}_samples_oldIMG -p 12) 2> {1}_sequences_{2}_samples.txt' ::: 10 100 1000 10000 ::: 10 100 1000

parallel -j 2 --progress '(/usr/bin/time -v picrust2_pipeline_oldIMG.py -s {1}_sequences.fasta -i {1}_sequences_{2}_samples.tsv -o picrust2_out_pipeline_{1}_sequences_{2}_samples_oldIMG_1thread -p 1) 2> {1}_sequences_{2}_samples_time_oldIMG_1thread.txt' ::: 10 ::: 10 100 1000

parallel -j 2 --progress '(/usr/bin/time -v picrust2_pipeline.py -s {1}_sequences.fasta -i {1}_sequences_{2}_samples.tsv -o picrust2_out_pipeline_{1}_sequences_{2}_samples_SC -p 12) 2> {1}_sequences_{2}_samples_time_SC.txt' ::: 10 100 1000 10000 ::: 10 100 1000

parallel -j 2 --progress '(/usr/bin/time -v picrust2_pipeline.py -s {1}_sequences.fasta -i {1}_sequences_{2}_samples.tsv -o picrust2_out_pipeline_{1}_sequences_{2}_samples_SC_1thread -p 1) 2> {1}_sequences_{2}_samples_time_SC_1thread.txt' ::: 10 ::: 10 100 1000
```

Added option for custom traits. Test this:
```{bash, eval=FALSE}
less /home/robyn/anaconda3/envs/picrust2-v2.6.0/bin/picrust2_pipeline.py
--custom_trait_tables_ref1

picrust2_pipeline.py -s chemerin_16S/seqs.fna -i chemerin_16S/table.biom -o picrust2_out_pipeline_custom_traits -p 12 --custom_trait_tables_ref1 ../add_custom_trait/bacteria_alkB.txt --custom_trait_tables_ref2 ../add_custom_trait/archaea_alkB.txt --no_pathways
```